...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_logged_in) {
    ...
    foreach ($columns as $col) {
        ...
        if (in_array($col, $textFields) && preg_match('/\d/', $value)) {
        ...
        // Телефон
        if ($col === 'phone_number' && $value !== '' && !preg_match('/^\+?[0-9\- ]+$/', $value)) {
            $errors[] = "Неверный формат номера телефона.";
            continue;
        }

        // Дата
        if (str_contains($col, 'date') && $value !== '' && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $value)) {
            $errors[] = "Поле '" . translate($col) . "' должно быть в формате YYYY-MM-DD.";
            continue;
        }

        // Время
        if ($col === 'departure_time' && $value !== '' && !preg_match('/^\d{2}:\d{2}(:\d{2})?$/', $value)) {
            $errors[] = "Поле '" . translate($col) . "' должно быть в формате HH:MM или HH:MM:SS.";
            continue;
        }

        // Интервал
        if ($col === 'flight_duration' && $value !== '' && !preg_match('/^\d+\s+(hour|minute|second|day)s?$/i', $value)) {
